...
class SasaranKinerjaForm(forms.ModelForm):
    ...
    pejabat_penilai = forms.ModelChoiceField(queryset=Account.objects.all(), required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user.unitkerja:
            ...
            self.fields["unor"].queryset = UnitKerja.objects.filter(id=user.unitkerja.id)
        

        if user.atasan:
            self.fields["nama_atasan"].initial = user.atasan.get_complete_name
            self.fields["jabatan_atasan"].initial = user.atasan.jabatan if user.atasan.jabatan else "---"
